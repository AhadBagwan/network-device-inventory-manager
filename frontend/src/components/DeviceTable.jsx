...
const DeviceTable = ({
  ...
  const isAllPaginatedSelected =
    ...
    paginatedDevices.every((d) => selectedIds.includes(d.id));
  ...
  return (
    <div className="relative noc-card rounded-xl overflow-hidden flex flex-col justify-between">
      {/* Floating Bulk Actions Toolbar */}
      {selectedIds.length > 0 && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3 px-4 py-2 rounded-2xl bg-slate-900/95 border border-cyan-500/40 text-xs font-mono shadow-2xl backdrop-blur-md animate-in slide-in-from-top-4 duration-200">
          <span className="font-bold text-cyan-400">
            {selectedIds.length} Selected
          </span>

          <button
            onClick={() => onBulkPing && onBulkPing(selectedIds)}
            className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 font-bold transition-all"
          >
          ...
          <button
            onClick={() => onBulkStatus && onBulkStatus(selectedIds, 'Maintenance')}
            className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 font-bold transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-rose-500/20 text-rose-400 border border-rose-500/30 hover:bg-rose-500/30 font-bold transition-all"
          >
            ...
            <tr className="bg-[var(--bg-main)] text-[var(--text-muted)] font-semibold uppercase tracking-wider border-b border-[var(--border-color)] sticky top-0 z-10">
              <th className="py-3 px-3 w-10 text-center">
                <input
                  ...
                  onChange={() =>
                    onToggleSelectAll &&
                    onToggleSelectAll(paginatedDevices.map((d) => d.id))
                  }
                  className="rounded border-[var(--border-color)] bg-[var(--bg-main)] text-[var(--accent-color)] focus:ring-0 cursor-pointer"
                />
              </th>
              <th
                onClick={() => onSort('hostname')}
                className="py-3 px-3 cursor-pointer hover:text-[var(--text-main)] transition-colors"
              >
              ...
              <th
                ...
                className="py-3 px-3 cursor-pointer hover:text-[var(--text-main)] transition-colors hidden md:table-cell"
              >
                Vendor {renderSortIcon('vendor')}
              </th>
              <th className="py-3 px-3 hidden lg:table-cell">Model</th>
              <th className="py-3 px-3 hidden sm:table-cell">Type</th>
              <th className="py-3 px-3 hidden xl:table-cell">Location</th>
              <th
              ...
              <th
                ...
                className="py-3 px-3 cursor-pointer hover:text-[var(--text-main)] transition-colors hidden sm:table-cell"
              >
                Latency {renderSortIcon('latency')}
              </th>
              <th
                onClick={() => onSort('last_checked')}
                className="py-3 px-3 cursor-pointer hover:text-[var(--text-main)] transition-colors hidden lg:table-cell"
              >
              ...
              paginatedDevices.map((device) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-3 font-bold text-[var(--text-main)]">
                      <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[var(--accent-color)] opacity-0 group-hover:opacity-100 transition-opacity" />
                        <span>{device.hostname}</span>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-cyan-400 font-semibold">{device.ip_address}</td>
                    <td className="py-3 px-3 font-sans text-[var(--text-main)] hidden md:table-cell">
                      {device.vendor}
                    </td>
                    <td className="py-3 px-3 font-sans text-[var(--text-muted)] hidden lg:table-cell">
                      {device.model}
                    </td>
                    <td className="py-3 px-3 font-sans text-[var(--text-muted)] hidden sm:table-cell">
                      <span className="noc-badge px-2 py-0.5 rounded text-[10px]">
                        {device.device_type}
                      </span>
                    </td>
                    <td className="py-3 px-3 font-sans text-[var(--text-muted)] hidden xl:table-cell truncate max-w-[150px]">
                      {device.location}
                    </td>
                    <td className="py-3 px-3 font-sans">{getStatusBadge(device.status)}</td>
                    ...
                    <td className="py-3 px-3 text-[var(--text-muted)] text-[11px] hidden lg:table-cell">
