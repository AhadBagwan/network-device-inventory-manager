...
const ForgotPassword = () => {
  ...
  const [step, setStep] = useState(1); // 1: Send OTP to Email, 2: Enter OTP Code from Email & New Password
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      toast.success(res.message || `OTP verification code sent to ${email}! Check your Gmail inbox.`);
      setStep(2);
      setTimer(60);
    } catch (err) {
  ...
  const handleResetPassword = async (e) => {
    ...
    if (!otp.trim() || otp.length !== 6) {
      toast.error('Please enter the 6-digit OTP code received in your email inbox.');
      return;
    }
        ...
        >
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-extrabold text-[var(--text-main)]">
              {step === 1 ? 'Forgot Password?' : 'Check Your Gmail / Email Inbox'}
            </h2>
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleSendOtp} className="space-y-4 text-xs">
              ...
              >
                <span>{isSubmitting ? 'Sending Email OTP...' : 'Send OTP to Gmail / Email'}</span>
                <HiArrowRight className="w-4 h-4" />
              </button>
            </form>
          ) : (
            /* STEP 2: VERIFY OTP FROM EMAIL & RESET PASSWORD FORM */
            <form onSubmit={handleResetPassword} className="space-y-4 text-xs">
              {/* OTP 6-digit Code Input */}
              <div>
                <label className="block font-semibold text-[var(--text-main)] mb-1">
                  6-Digit OTP Code (Sent to Email) <span className="text-rose-400">*</span>
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[var(--text-muted)]">
              ...
              <div className="flex items-center justify-between text-[11px] font-mono text-[var(--text-muted)] pt-1">
                ...
                >
                  ...
                  <span>{timer > 0 ? `Resend in ${timer}s` : 'Resend Email OTP'}</span>
                </button>
              </div>
